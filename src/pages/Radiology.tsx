...
import { Loader2, Upload, AlertCircle, Download, Image, X } from "lucide-react";
...
const Radiology = () => {
  ...
  const [radiologyResult, setRadiologyResult] =
    useState<RadiologyResult | null>(null);
  ...
  const parseRadiologyResponse = (response: string): RadiologyResult => {
    try {
      ...
      if (findings.length === 0) {
        findings.push(
          "No significant abnormalities detected on initial review"
        );
      }
      ...
      if (!impression) {
        impression =
          "Radiological findings require clinical correlation for complete assessment";
      }

      console.log("Parsed radiology result:", {
        findings,
        conditions,
        recommendedTests,
        impression,
      ...
      return {
        ...
        impression:
          "Could not parse results accurately. Please consult a medical professional.",
      };
    }
  };
    ...
    const htmlContent = `
      ...
      <html>
        ...
        <body>
          ...
          <ul>
            ${radiologyResult.recommendedTests
              .map((test) => `<li>${test}</li>`)
              .join("")}
          </ul>
                      ...
                      <div className="border rounded-md p-4">
                        ...
                        <ul className="space-y-1">
                          {radiologyResult.findings.map((finding, idx) => (
                            <li
                              key={idx}
                              className="text-sm text-muted-foreground flex items-baseline"
                            >
                      ...
                      <div className="border rounded-md p-4">
                        ...
                        <ul className="space-y-1">
                          {radiologyResult.conditions.map((condition, idx) => (
                            <li
                              key={idx}
                              className={`text-sm flex items-baseline ${getSeverityColor(
                                condition
                              )}`}
                            >
                              <span className="mr-2">•</span>
                    ...
                    >
                      ...
                      <Alert>
                        ...
                        <AlertDescription className="text-sm">
                          This AI analysis is meant to assist healthcare
                          professionals and should not replace clinical
                          judgment. Results should be verified by a qualified
                          radiologist.
                        </AlertDescription>
                      </Alert>
                    </TabsContent>
